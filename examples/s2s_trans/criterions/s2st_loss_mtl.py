...
class Tacotron2CriterionConfig(FairseqDataclass):
    ...
    ctc_weight_tgt: float = field(
        default=0.0, metadata={"help": "weight for CTC loss for target language"}
    )
...
@register_criterion("s2st_loss_mtl", dataclass=Tacotron2CriterionConfig)
class Tacotron2Criterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, n_frames_per_step,
                 ...
                 bce_pos_weight, ctc_weight, ctc_weight_tgt):
        super().__init__(task)
        ...
        self.ctc_weight_tgt = ctc_weight_tgt
        if self.ctc_weight > 0 or self.ctc_weight_tgt > 0:
    ...
    def forward(self, model, sample, reduction="mean"):
        ...
        tgt_text_len = sample["tgt_text_len"]
        ...
        ctc_loss_tgt = torch.tensor(0.).type_as(l1_loss)
        if self.ctc_weight > 0.:
            net_output = (feat_out, eos_out, extra["out_middle_layers"]) 
            for k in list([int(k) for k in self.args.conv_kernel_sizes.split(",")]):
                    src_lens = (src_lens - k + 2*(k//2)) // 2 + 1 
            lprobs = model.get_normalized_probs(net_output, log_probs=True, tag="ctc")
        ...
        if self.ctc_weight_tgt > 0.:
            net_output = (feat_out, eos_out, extra["out_middle_layers_decoder"]) 
            # for k in list([int(k) for k in self.args.conv_kernel_sizes.split(",")]):
            #         src_lens = (src_lens - k + 2*(k//2)) // 2 + 1 
            lprobs = model.get_normalized_probs(net_output, log_probs=True, tag="ctc_tgt")
            lprobs = lprobs.transpose(0, 1)  # T x B x C
            
            tgt_mask = lengths_to_mask(tgt_text_len)
            tgt_tokens_flat = tgt_text.masked_select(tgt_mask)
            # ctc_loss = F.ctc_loss(
            #     lprobs, src_tokens_flat, src_lens, src_text_len,
            #     reduction=reduction, zero_infinity=True
            # ) * self.ctc_weight
            with torch.backends.cudnn.flags(deterministic=True):
                ctc_loss_tgt = self.ctc_loss(lprobs, tgt_tokens_flat, tgt_lens, tgt_text_len) * self.ctc_weight_tgt

        loss = l1_loss + mse_loss + eos_loss + attn_loss + ctc_loss + ctc_loss_tgt
        ...
        logging_output = {
            ...
            "ctc_loss_tgt": utils.item(ctc_loss_tgt.data)
        }
    ...
    def reduce_metrics(cls, logging_outputs: List[Dict[str, Any]]) -> None:
        ...
        for key in ["loss", "l1_loss", "mse_loss", "eos_loss", "attn_loss", "ctc_loss", "ctc_loss_tgt"]:
