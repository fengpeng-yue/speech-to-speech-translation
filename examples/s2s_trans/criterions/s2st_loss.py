...
class Tacotron2CriterionConfig(FairseqDataclass):
    ...
    asr_ce_weight: float = field(
        default=0.0, metadata={"help": "weight for asr CE loss"}
    )
    st_ce_weight: float = field(
        default=0.0, metadata={"help": "weight for sr CE loss"}
    )
    l1_loss_weight: float = field(
        default=1.0, metadata={"help": "weight for l1 loss"}
    )
    mse_loss_weight: float = field(
        default=1.0, metadata={"help": "weight for mse loss"}
    )
    eos_loss_weight: float = field(
        default=1.0, metadata={"help": "weight for eos loss"}
    )
    attn_loss_weight: float = field(
        default=1.0, metadata={"help": "weight for attention loss"}
    )
...
@register_criterion("s2st_loss", dataclass=Tacotron2CriterionConfig)
class Tacotron2Criterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, n_frames_per_step,
                 ...
                 bce_pos_weight, ctc_weight, asr_ce_weight, st_ce_weight, l1_loss_weight, 
                 mse_loss_weight, eos_loss_weight, attn_loss_weight, label_smoothing,
        ...
        self.asr_ce_weight = asr_ce_weight
    ...
    def forward(self, model, sample, reduction="mean"):
        ...
        if self.asr_ce_weight > 0:
            prev_src_text_tokens = sample["net_input"]["prev_src_text_tokens"]
        else:
        ...
        main_task_output,aux_asr_output, aux_st_output = model(
        ...
        aux_st_loss = torch.tensor(0.).type_as(l1_loss)
        if self.st_ce_weight > 0:
            #print("src_text_token % d" % src_txt_ntokens)
            aux_st_loss, st_nll_loss = self.compute_aux_loss(model.aux_st_decoder.get_normalized_probs, model.get_targets , aux_st_output, sample,"st")
            aux_st_loss = aux_st_loss / tgt_txt_ntokens  * self.st_ce_weight
        l1_loss, mse_loss, eos_loss, attn_loss =  l1_loss*self.l1_loss_weight, mse_loss*self.mse_loss_weight, eos_loss*self.eos_loss_weight, attn_loss*self.attn_loss_weight
        loss = l1_loss + mse_loss + eos_loss + attn_loss + ctc_loss + aux_asr_loss  + aux_st_loss
        ...
        logging_output = {
            ...
            "aux_asr_loss": utils.item(aux_asr_loss.data),
            "aux_st_loss": utils.item(aux_st_loss.data),
        }
        ...
        if self.report_accuracy and self.st_ce_weight > 0:
            n_correct, total = self.compute_accuracy(model.aux_st_decoder.get_normalized_probs, model.get_targets ,aux_st_output, "st",sample) 
    ...
    def reduce_metrics(cls, logging_outputs: List[Dict[str, Any]]) -> None:
        ...
        for key in ["loss", "l1_loss", "mse_loss", "eos_loss", "attn_loss", "ctc_loss","aux_asr_loss","aux_st_loss"]:
