...
from examples.s2s_trans.data.s2st_dataset import S2STDatasetCreator
from fairseq.tasks import register_task
from fairseq.tasks.speech_to_text import SpeechToTextTask
from fairseq.data import Dictionary
from examples.s2s_trans.data.data_cfg import S2STDataConfig
from fairseq.speech_generator_for_s2st import (
...
def convertobool(argument):
    if argument == "false":
        return False
    elif argument == "true":
        return True
    else:
        NotImplementedError("Input type error")

@register_task('s2s_translation')
class S2ST_TranslationTask(SpeechToTextTask):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument('--max-source-positions', default=3000, type=int,
                            metavar='N',
                            help='max number of tokens in the source sequence')
        parser.add_argument('--max-target-positions', default=2400, type=int,
                            metavar='N',
        ...
        parser.add_argument("--use-hubert", type=str, default="false", help="use hubert as the feature extractor")
        parser.add_argument("--kd-encoder", type=str, default="false", help="for kd encoder")
        parser.add_argument("--input-text", type=str, default="false", help="for text to speech translation")
        parser.add_argument("--speaker-to-id", type=str, default=None, help="use speaker feature")

    def __init__(self, args, src_dict, tgt_dict):
        super().__init__(args, src_dict)
        self.src_dict = src_dict
        self.tgt_dict = tgt_dict
        self.data_cfg = S2STDataConfig(Path(args.data) / args.config_yaml)
        self.sr = self.data_cfg.config.get("features").get("sample_rate")
        self.args.use_hubert = convertobool(args.use_hubert)
        self.args.input_text = convertobool(args.input_text)
        self.args.kd_encoder = convertobool(args.kd_encoder)
        if args.speaker_to_id is not None:
            self.speaker_to_id = json.loads(args.speaker_to_id)
    ...
    def setup_task(cls, args, **kwargs):
        data_cfg = S2STDataConfig(Path(args.data) / args.config_yaml)
        src_dict_path = Path(args.data) / data_cfg.src_vocab_filename
        tgt_dict_path = Path(args.data) / data_cfg.tgt_vocab_filename
        ...
        logger.info(
            f"ST dictionary size ({data_cfg.tgt_vocab_filename}): " f"{len(tgt_dict):,}"
        )
        ...
        logger.info(
            f"ASR dictionary size ({data_cfg.src_vocab_filename}): " f"{len(src_dict):,}"
        )
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        print(self.args.use_hubert)
        self.data_cfg.set_use_hubert(self.args.use_hubert)
        self.data_cfg.set_kd_encoder(self.args.kd_encoder)
        self.datasets[split] = S2STDatasetCreator.from_tsv(
            self.args.data, self.data_cfg, split, 
    ...
    @classmethod
    def get_speaker_embeddings(cls, args, speaker_embed_dim):
        embed_speaker = None
        if args.speaker_to_id is not None:
            if args.speaker_emb_path is None:
                embed_speaker = torch.nn.Embedding(
                    len(args.speaker_to_id), speaker_embed_dim
                )
            else:
                speaker_emb_mat = np.load(args.speaker_emb_path)
                assert speaker_emb_mat.shape[1] == speaker_embed_dim
    ...
    def build_model(self, cfg):
        ...
        if getattr(cfg, "eval_inference", False):
            self.generator = self.build_generator_tts([model], cfg)
        return model

    def build_generator_tts(self, models, cfg, vocoder=None, **unused):
        if vocoder is None:
