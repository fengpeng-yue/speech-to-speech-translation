...
AUDIO_MANIFEST_ROOT=/path/fisher_audio
FEATURE_MANIFEST_ROOT=/path/fisher_feature
...
s2st_exp_name=fisher_mls_mix_mls_only_translatotron_baseline
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    python3 -m fairseq_cli.train ${FEATURE_MANIFEST_ROOT} --save-dir ${SAVE_DIR}/st_pretraining/ \
    --user-dir /opt/tiger/s2st_fairseq/examples/s2s_trans \
    --config-yaml config.yaml --train-subset train --valid-subset dev \
    --num-workers 4 --max-tokens ${s2st_max_tokens} --max-update 300000 \
    ...
    --optimizer adam --lr 2e-3 --lr-scheduler inverse_sqrt --warmup-updates ${s2st_warmup_updates} \
    ...
    --label-smoothing 0.1 --asr-ce-weight 1 --st-ce-weight 1 --report-accuracy \
