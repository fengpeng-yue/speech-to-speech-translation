...
class S2STDataConfig(object):
    ...
    def tgt_vocab_filename(self):
        """fairseq vocabulary file under data root"""
        return self.config.get("tgt_vocab_filename", "dict.txt")
    ...
    def set_use_hubert(self,use_hubert):
        self.use_hubert = use_hubert
    def set_kd_encoder(self,kd_encoder):
        self.kd_encoder = kd_encoder
    ...
    def get_feature_transforms(self, split, is_train):
        ...
        cfg["transforms"] = cur
        return cfg
    def get_feature_transforms_for_src(self, split, is_train):
        """Split-specific feature transforms. Allowing train set
        wildcard `_train`, evaluation set wildcard `_eval` and general
        wildcard `*` for matching."""
        from copy import deepcopy
        ...
        cfg = deepcopy(self.config)
        _cur = cfg.get("src_transforms", {})
        cur = _cur.get(split)
        cur = _cur.get("_train") if cur is None and is_train else cur
        cur = _cur.get("_eval") if cur is None and not is_train else cur
        cur = _cur.get("*") if cur is None else cur
        cfg["src_transforms"] = cur
        return cfg
    def get_feature_transforms_for_tgt(self, split, is_train):
        """Split-specific feature transforms. Allowing train set
        wildcard `_train`, evaluation set wildcard `_eval` and general
        wildcard `*` for matching."""
        from copy import deepcopy
        ...
        cfg = deepcopy(self.config)
        _cur = cfg.get("tgt_transforms", {})
        cur = _cur.get(split)
        cur = _cur.get("_train") if cur is None and is_train else cur
        cur = _cur.get("_eval") if cur is None and not is_train else cur
        cur = _cur.get("*") if cur is None else cur
        cfg["tgt_transforms"] = cur
        return cfg

    @property
    def src_global_cmvn_stats_npz(self) -> Optional[str]:
        path = self.config.get("src_global_cmvn", {}).get("stats_npz_path", None)
        return self._auto_convert_to_abs_path(path)

    @property
    def tgt_global_cmvn_stats_npz(self) -> Optional[str]:
        path = self.config.get("tgt_global_cmvn", {}).get("stats_npz_path", None)
        return self._auto_convert_to_abs_path(path)
