...
def make_parser():
    ...
    parser.add_argument("--decode-source-text", action="store_true")
    parser.add_argument("--decode-target-text", action="store_true")
    parser.add_argument("--decode-target-mel", action="store_true")
    return parser
...
def main(args):
    ...
    asr_score = scoring.build_scorer(args.scoring, model.decoder.src_dict)
    fsrc_texts = open(f"{args.results_path}/src_texts.txt", 'w')
    fhyps_src_texts = open(f"{args.results_path}/hyps_src_texts.txt", 'w')

    # st_score = scoring.build_scorer('sacrebleu', model.decoder.tgt_dict)
    # ftgt_texts = open(f"{args.results_path}/tgt_texts.txt", 'w')
    # fhyps_tgt_texts = open(f"{args.results_path}/hyps_tgt_texts.txt", 'w')

    with progress_bar.build_progress_bar(args, itr) as t:
        for sample in t:
            # import pdb;pdb.set_trace()
            sample = utils.move_to_cuda(sample) if use_cuda else sample
            hypos = generator.generate(model, sample, has_targ=args.dump_target, decode_source_text=args.decode_source_text, decode_target_mel=args.decode_target_mel)
            if args.decode_source_text:
                for hypo in hypos:
                    asr_score.add_string(hypo["src_texts"], hypo["hyps_src_texts"])
                    fsrc_texts.write(hypo["src_texts"]+'\n')
                    fhyps_src_texts.write(hypo["hyps_src_texts"]+'\n')
            if args.decode_target_mel:
    ...
    if args.decode_source_text:
        print(f"WER: {asr_score.score()}")
        fsrc_texts.close()
        fhyps_src_texts.close()
