...
s2st_exp_name=fisher_translatotron_baseline
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    python3 -m examples.s2s_trans.preprocessing.get_feature_manifest \
    --audio-manifest-root ${AUDIO_MANIFEST_ROOT} \
    --output-root ${FEATURE_MANIFEST_ROOT} \
    --ipa-vocab --use-g2p \
    --splits "fisher_dev2" "fisher_dev" "fisher_test" "fisher_train"
fi
