...
def process(args):
    ...
    src_gcmvn_npz_path = out_root / "src_gcmvn_stats.npz"
    ...
    if src_zip_path.exists() and src_gcmvn_npz_path.exists() and \
        ...
        for sample in tqdm(samples):
            idx += 1
            if idx > (int(args.processor)-1)*2000 and idx <= int(args.processor)*2000:
        ...
        if not args.only_extract_audio_feature:   
    ...
    if not args.only_extract_audio_feature:
        ...
        if  args.ipa_vocab:
            src_vocab = Counter()
            for t in manifest_by_split["fisher_train"]["src_text"]:
            ...
            tgt_vocab = Counter()
            for t in manifest_by_split["fisher_train"]["tgt_text"]:
...
def main():
    ...
    parser.add_argument("--win-length", type=int, default=1200)
    ...
    parser.add_argument("--only-extract-audio-feature", type=bool, default=True)
