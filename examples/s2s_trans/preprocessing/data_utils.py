...
def gen_config_yaml(
    ...
    src_vocab_name: Optional[str] = None,
    tgt_vocab_name: Optional[str] = None,
    ...
    src_gcmvn_path: Optional[Path] = None,
    tgt_gcmvn_path: Optional[Path] = None,
    extra=None
):
    manifest_root = manifest_root.absolute()
    writer = S2TDataConfigWriter(manifest_root / yaml_filename)
    assert spm_filename is not None or src_vocab_name is not None
    src_vocab_name = spm_filename.replace(".model", ".txt") if src_vocab_name is None \
        else src_vocab_name
    writer.set_src_vocab_filename(src_vocab_name)
    ...
    if cmvn_type == "global":
        if src_gcmvn_path is None:
            ...
            writer.set_src_global_cmvn(src_gcmvn_path.as_posix())

        if tgt_gcmvn_path is None:
            raise ValueError("Please provide path of global cmvn file.")
        else:
            writer.set_tgt_global_cmvn(tgt_gcmvn_path.as_posix())
...
class S2TDataConfigWriter(object):
    ...
    def set_src_vocab_filename(self, vocab_filename: str = "dict.txt"):
        self.config["src_vocab_filename"] = vocab_filename
    def set_tgt_vocab_filename(self, vocab_filename: str = "dict.txt"):
    ...
    def set_src_global_cmvn(self, stats_npz_path: str):
        self.config["src_global_cmvn"] = {"stats_npz_path": stats_npz_path}
