...
class AutoRegressiveSpeechGenerator(SpeechGenerator):
    def __init__(
            ...
            eos_prob_threshold: float = 0.5, input_text: bool=False,
    ):
    ...
    def generate(self, model, sample, has_targ=False, **kwargs):
        ...
        bsz, src_len, _ = src_tokens.size() if src_tokens is not None else  collated_audios.unsqueeze(-1).size()
        ...
        if self.input_text:
            ...
            src_lengths = sample["src_text_len"]
            encoder_out = model.forward_encoder(src_tokens, src_lengths,
                                            speaker=sample["speaker"])
        else:
            encoder_out = model.forward_encoder(src_tokens, src_lengths,
                                            collated_audios,padding_mask,
                                            speaker=sample["speaker"])
        incremental_state = {}
        feat, attn, eos_prob = [], [], []
        if src_tokens is None:
            src_tokens = collated_audios
        ...
        #     speaker_embedding = model.decoder.embed_speaker(sample["speaker"])
        #     prev_feat_out = speaker_embedding
        # else:
        #     prev_feat_out = encoder_out["encoder_out"][0].new_zeros(bsz, 1, out_dim) # bos embedding
        prev_feat_out = encoder_out["encoder_out"][0].new_zeros(bsz, 1, out_dim) # bos embedding
...
class TeacherForcingAutoRegressiveSpeechGenerator(AutoRegressiveSpeechGenerator):
    ...
    def generate(self, model, sample, has_targ=False, **kwargs):
        ...
        collated_audios = sample["net_input"]["collated_audios_orig"]
        padding_mask =  sample["net_input"]["padding_mask"]
        bsz, src_lens, _ = src_tokens.size() if src_tokens is not None else  collated_audios.unsqueeze(-1).size()
        ...
        main_task_output,aux_asr_output, aux_st_output = model(
            src_tokens, src_lengths,collated_audios, padding_mask, prev_out_tokens,
        ...
        feat, eos_prob, extra = main_task_output
