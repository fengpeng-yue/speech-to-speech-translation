...
@register_model("s2t_transformer_hubert")
class S2TTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--ctc-loss",
            action="store_true",
            help="use ctc method or attention method",
        )
        parser.add_argument(
            "--conv-kernel-sizes",
            type=str,
    ...
    def build_hubert(cls, args):
        ...
        if pretraining_path is not None and args.use_hubert:
    ...
    def build_encoder(cls, args,task):
        ...
        encoder = S2TTransformerEncoder(args, hubert)
    ...
    def forward(self, src_tokens, src_lengths,collated_audios, padding_mask,prev_output_tokens):
        """
        ...
        #     # print(hubert_out.size())
        #     # print(input_lengths)
        #     encoder_out = self.encoder(src_tokens=hubert_out, src_lengths=input_lengths)
        # else:
        encoder_out = self.encoder(src_tokens=src_tokens, src_lengths=src_lengths,collated_audios=collated_audios, padding_mask=padding_mask)
...
class S2TTransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths,collated_audios, padding_mask, return_all_hiddens=False):
        if self.hubert is not None:
            self.hubert.eval()
            with torch.no_grad():
                hubert_out, padding_mask = self.hubert.extract_features(collated_audios,padding_mask)
...
@register_model_architecture(model_name="s2t_transformer_hubert", arch_name="s2t_transformer_hubert")
...
@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_s")
...
@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_xs")
...
@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_sp")
def s2t_transformer_sp(args):
    args.encoder_layers = getattr(args, "encoder_layers", 16)
    s2t_transformer_s(args)


@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_m")
...
@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_mp")
def s2t_transformer_mp(args):
    args.encoder_layers = getattr(args, "encoder_layers", 16)
    s2t_transformer_m(args)


@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_l")
...
@register_model_architecture("s2t_transformer_hubert", "s2t_transformer_hubert_lp")
