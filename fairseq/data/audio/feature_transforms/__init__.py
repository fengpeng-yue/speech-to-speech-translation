...
class CompositeAudioFeatureTransform(AudioFeatureTransform):
    ...
    @classmethod
    def from_config_dict_for_src(cls, config=None):
        _config = {} if config is None else config
        _transforms = _config.get("src_transforms")
        if _transforms is None:
            return None
        transforms = [
            get_audio_feature_transform(_t).from_config_dict(_config.get(_t))
        ...
        return CompositeAudioFeatureTransform(transforms)
    @classmethod
    def from_config_dict_for_tgt(cls, config=None):
        _config = {} if config is None else config
        _transforms = _config.get("tgt_transforms")
        if _transforms is None:
            return None
        transforms = [
            get_audio_feature_transform(_t).from_config_dict(_config.get(_t))
