...
class GlobalCMVN(AudioFeatureTransform):
    ...
    def __call__(self, x):
        ...
        return x
@register_audio_feature_transform("src_global_cmvn")
class SRCGlobalCMVN(AudioFeatureTransform):
    """Global CMVN (cepstral mean and variance normalization). The global mean
    and variance need to be pre-computed and stored in NumPy format (.npz)."""
    ...
    def from_config_dict(cls, config=None):
        _config = {} if config is None else config
        return SRCGlobalCMVN(_config.get("stats_npz_path"))
    ...
    def __call__(self, x):
        ...
        x = np.divide(x, self.std)
        return x
@register_audio_feature_transform("tgt_global_cmvn")
class TGTGlobalCMVN(AudioFeatureTransform):
    """Global CMVN (cepstral mean and variance normalization). The global mean
    and variance need to be pre-computed and stored in NumPy format (.npz)."""
    ...
    def from_config_dict(cls, config=None):
        _config = {} if config is None else config
        print(_config)
        return TGTGlobalCMVN(_config.get("stats_npz_path"))
