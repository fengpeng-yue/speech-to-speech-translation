...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        elif "src_speech" in net_input:
            if net_input["src_speech"] is not None:
                src_tokens = net_input["src_speech"]
                src_lengths = (
                    (src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
                )
            else:
                src_tokens = net_input["collated_audios_orig"]
                src_lengths = net_input["padding_mask"].size(-1) - net_input["padding_mask"].sum(-1)
